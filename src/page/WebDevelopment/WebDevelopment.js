...
import WebDevelopmentTransitionController from './WebDevelopmentTransitionController';
...
export default {
  ...
  methods: {
    ...
    startSlider() {
      this.slideTimer = setInterval(() => {
        ...
        if (this.currentSlide > 6) {
          ...
          TweenLite.to(this.$refs.slide6, 0.8, {opacity: 0});
        } else {
    ...
    handleAllComponentsReady() {
      this.transitionController = new WebDevelopmentTransitionController(this);
      this.isReady();
    },
    handleScroll(event) {
      if (event.deltaY < 0) {
